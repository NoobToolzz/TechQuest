...
Public Class SimpleHttpServer
    ...
    Public Async Sub Start(pageType As String, yourTableResults As String, leaderboardTableResults As String)
        listener.Start()
        Dim html As String
        ...
        ElseIf pageType = "resultPage" Then
            If String.IsNullOrEmpty(yourTableResults) Or String.IsNullOrEmpty(leaderboardTableResults) Then
                Throw New ArgumentException("HTML text must be provided for resultPage")
            End If
            html = $"<!DOCTYPE html><html lang='en'><head> <meta charset='UTF-8'> <meta name='viewport' content='width=device-width, initial-scale=1.0'> <title>TechQuest Results</title> <style> body {{ font-family: Arial, sans-serif; background: linear-gradient(to right, #4e54c8, #8f94fb); color: #333; margin: 0; padding: 0; display: flex; justify-content: center; align-items: center; height: 100vh; overflow: hidden; position: relative; }} .container {{ display: flex; flex-direction: column; background: rgba(255, 255, 255, 0.8); border-radius: 15px; box-shadow: 0 8px 30px rgba(0, 0, 0, 0.3); padding: 30px; max-width: 1500px; margin: 20px; opacity: 0; transform: translateY(-20px); animation: fadeInUp 0.8s forwards; }} .header {{ font-size: 2em; color: #4e54c8; text-align: center; margin: 0; }} .horizontal-divider {{ width: 100%; height: 2px; background-color: #4e54c8; margin: 5px 0; }} .content {{ display: flex; margin-top: 20px; }} .divider {{ width: 2px; background-color: #4e54c8; margin: 0 20px; height: auto; box-shadow: 0 0 10px rgba(0, 0, 0, 0.2); }} @keyframes fadeInUp {{ to {{ opacity: 1; transform: translateY(0); }} }} .left-section {{ flex: 1; padding-right: 20px; text-align: left; }} .right-section {{ flex: 1; padding-left: 20px; text-align: left; }} h2 {{ font-size: 1.5em; margin: 20px 0; color: #333; }} p {{ font-size: 1.1em; line-height: 1.6; margin: 10px 0; }} table {{ width: 100%; border-collapse: collapse; margin-top: 20px; }} th, td {{ border: 1px solid #4e54c8; padding: 10px; text-align: left; }} th {{ background-color: #4e54c8; color: white; }} .notification {{ position: fixed; top: 20px; right: -300px; background: linear-gradient(to right, #ff7e5f, #feb47b); color: white; padding: 10px 15px; border-radius: 10px; box-shadow: 0 4px 20px rgba(0, 0, 0, 0.2); display: flex; align-items: center; z-index: 1000; opacity: 0; transition: right 1s ease, opacity 1s ease; width: 200px; }} .icon {{ font-size: 1.5em; margin-right: 10px; }} .notification-content {{ display: flex; flex-direction: column; }} .notification-content h3 {{ margin: 0; font-size: 1.1em; }} .notification-content p {{ margin: 0; font-size: 0.9em; }} .dismiss-button {{ background: transparent; border: none; color: white; font-size: 1.2em; cursor: pointer; margin-left: auto; }} </style></head><body> <div class='container'> <h1 class='header'>TechQuest Global Statistics</h1> <div class='horizontal-divider'></div> <div class='content'> <div class='left-section'> <h2>Your Results</h2> {yourTableResults} </div> <div class='divider'></div> <div class='right-section'> <h2>Global Leaderboard</h2> {leaderboardTableResults} </div> </div> </div> <div class='notification' id='notification'> <button class='dismiss-button' id='dismissButton'>✖</button> <div class='icon'>🔔</div> <div class='notification-content'> <h3>Hello!</h3> <p>Close this window to exit the program</p> </div> </div> <script> const notification = document.getElementById('notification'); const dismissButton = document.getElementById('dismissButton'); function showNotification() {{ notification.style.right = '20px'; notification.style.opacity = '1'; }} setTimeout(() => {{ showNotification(); }}, 1500); dismissButton.addEventListener('click', () => {{ notification.style.right = '-300px'; notification.style.opacity = '0'; }}); </script></body></html>"
        Else
